...
def prompt_claude_directly_raw(prompt: str, system_prompt: Optional[str] = None, temperature: Optional[float] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Execute the command
        logger.info(f"Sending prompt directly to Claude CLI (raw mode): {prompt[:50]}...")
        logger.info(f"Using arguments: {claude_args}")
        ...
        try:
            ...
            try:
                ...
                read_tool = FunctionTool(
                    function=read_file_directly,
                    function_schema=read_file_schema
                )
                write_tool = FunctionTool(
                    function=write_file_directly,
                ...
                execute_tool = FunctionTool(execute_command_directly, schema=execute_command_schema)

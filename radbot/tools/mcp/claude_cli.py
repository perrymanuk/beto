...
def execute_command_via_claude(command: str, working_dir: Optional[str] = None) -> Dict[str, Any]:
    """
        ...
        try:
            ...
            try:
                ...
                read_tool = FunctionTool(
                    function=read_file_via_claude,
                    function_schema=read_file_schema
                )
                write_tool = FunctionTool(
                    function=write_file_via_claude,
                    function_schema=write_file_schema
                )
                logger.info("Created Claude CLI MCP tools with function_schema")
            except TypeError:
                # Try with schema (older ADK)
                execute_tool = FunctionTool(execute_command_via_claude, schema=execute_command_schema)
                read_tool = FunctionTool(read_file_via_claude, schema=read_file_schema)
                write_tool = FunctionTool(write_file_via_claude, schema=write_file_schema)
...
def main():
    ...
    if cmd_result.get("success", False):
        ...
        print(f"Output: {cmd_result.get('output', '')[:200]}..." if len(cmd_result.get('output', '')) > 200 else f"Output: {cmd_result.get('output', '')}")
    else:
        print(f"❌ Command execution failed: {cmd_result.get('error', 'Unknown error')}")

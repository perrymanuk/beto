...
def resolve_library_id(library_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            logger.warning(f"Unexpected result format from resolve-library-id tool: {result}")
            return {
                "success": False,
...
def get_library_docs(library_id: str, topic: Optional[str] = None, tokens: Optional[int] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            # Handle unexpected result format
            logger.warning(f"Unexpected result format from get-library-docs tool: {result}")
            return {
                "success": False,
                "error": "Unexpected result format",
                "result": str(result)
        ...
        try:
            ...
            try:
                resolve_tool = FunctionTool(
                    ...
                    function_schema=resolve_library_schema
                )
                docs_tool = FunctionTool(
                    function=get_library_docs,
                    function_schema=get_docs_schema
                )
                logger.info("Created Context7 MCP tools with function_schema")
            except TypeError:
                # Try with schema (older ADK)
                resolve_tool = FunctionTool(resolve_library_id, schema=resolve_library_schema)
...
def main():
    ...
    if resolve_result.get("success", False):
        ...
        if docs_result.get("success", False):
            ...
            print(f"Documentation snippet: {docs_result.get('documentation', '')[:200]}...")
        else:
            print(f"❌ Documentation retrieval failed: {docs_result.get('error', 'Unknown error')}")
    else:
        print(f"❌ Library resolution failed: {resolve_result.get('error', 'Unknown error')}")

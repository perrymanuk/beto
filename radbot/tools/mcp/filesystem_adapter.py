...
from radbot.filesystem.adapter import create_fileserver_toolset as direct_create_fileserver_toolset
...
def create_fileserver_toolset() -> List[FunctionTool]:
    ...
    try:
        ...
        if tools:
            ...
            logger.info(f"Successfully created {len(tools)} filesystem tools: {', '.join(tool_names)}")
        else:

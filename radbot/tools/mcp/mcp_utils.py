...
def test_home_assistant_connection() -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "tools": tool_names,
            "tool_details": tool_infos[:5]  # Include details for the first 5 tools only
        }
        
        # Extract common domains as well
        domains = set()
        common_domains = [
            'light', 'switch', 'sensor', 'climate', 'media', 
            'vacuum', 'scene', 'cover', 'fan', 'lock', 'input'
        ]
...
async def _find_home_assistant_entities_async(search_term: str, domain_filter: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Available Home Assistant tools for entity search: {', '.join(tool_names[:10])}...")
            ...
            for tool in ha_tools:
                ...
                if ('get_entities' in tool_name.lower() or 
                    'list_entities' in tool_name.lower() or
                    'entities.list' in tool_name.lower() or
                    'entity.list' in tool_name.lower()):
            ...
            if get_entities_tool:
                try:
                    logger.info(f"Calling entity listing tool: {getattr(get_entities_tool, 'name', 'unknown')}")
                    ...
                    if isinstance(entities, list):
                        ...
                        if "entity_ids" in entities:
                            ...
                            logger.info(f"Found {len(entity_ids)} entities in entity_ids key")
                        elif "entities" in entities:
                            entity_ids = entities["entities"]
                            logger.info(f"Found {len(entity_ids)} entities in entities key")
                            ...
                            possible_entities = [key for key in entities.keys() if '.' in key]
                            if possible_entities:
                                entity_ids = possible_entities
                                logger.info(f"Extracted {len(entity_ids)} possible entity IDs from dict keys")
                except Exception as e:
            ...
            if not entity_ids:
                logger.info("No entities found with entity listing tool, trying to extract from domains")
        ...
        for entity_id in entity_ids:
            ...
            if score > 0:
                domain, name = entity_id.split(".", 1) if "." in entity_id else ("unknown", entity_id)
                matches.append({
                    "entity_id": entity_id,
                    "domain": domain,
                    "name": name,
                    "score": score,
                    "match_level": "exact" if score == len(search_parts) else "partial"
                })
...
def find_home_assistant_entities(search_term: str, domain_filter: Optional[str] = None) -> Dict[str, Any]:
    """
    Search for Home Assistant entities by name or partial match.
    ...
    try:
        ...
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(_find_home_assistant_entities_async(search_term, domain_filter))
        loop.close()
        return result
    except Exception as e:

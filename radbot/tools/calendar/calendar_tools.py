...
class CalendarEvent(BaseModel):
    ...
    attendees: Optional[List[Dict[str, Any]]] = Field(None, description="List of attendees")
...
def get_calendar_manager() -> CalendarManager:
    ...
    if _calendar_manager is None:
        ...
        if not auth_success:
            print("WARNING: Calendar authentication failed. Calendar operations will not work.")
...
def create_calendar_event_wrapper(
    ...
    try:
        ...
        if result is None:
            error_message = "Failed to create event. Check calendar authentication and permissions."
            print(error_message)
            return {"status": "error", "message": error_message}
        elif isinstance(result, dict) and 'error' in result:
            error_message = f"Failed to create event: {result.get('error', 'Unknown error')}"
            print(error_message)
            return {"status": "error", "message": error_message}
...
def update_calendar_event_wrapper(
    ...
    try:
        ...
        if result is None:
            ...
            error_message = f"Failed to update event: {result.get('error', 'Unknown error')}"
            print(error_message)
            return {"status": "error", "message": error_message}
    ...
    try:
        ...
        if isinstance(result, dict):
            if "success" in result and result["success"]:
                ...
                error_message = f"Failed to delete event: {result.get('error', 'Unknown error')}"
                print(error_message)
                return {"status": "error", "message": error_message}

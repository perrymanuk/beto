...
def create_memory_enabled_agent(
    ...
    if not memory_service:
        ...
        try:
            ...
            logging.info(f"Successfully initialized QdrantMemoryService with collection '{collection}'")
            ...
            if hasattr(memory_service, 'client') and memory_service.client:
                ...
                logging.debug(f"Memory service client: {client_info}, collection: {collection_name}")
    ...
    if not hasattr(agent, '_memory_service') or agent._memory_service is None:
        logging.warning("Memory service not properly set on agent, setting it explicitly")
    ...
    if agent.root_agent and agent.root_agent.tools:
        ...
        for t in agent.root_agent.tools:
            if (hasattr(t, '__name__') and t.__name__ in ['search_past_conversations', 'store_important_information']) or \
               (hasattr(t, 'name') and t.name in ['search_past_conversations', 'store_important_information']):
                name = getattr(t, '__name__', getattr(t, 'name', str(t)))
                memory_tools_found.append(name)
        
        if memory_tools_found:
            logging.info(f"Memory tools successfully registered: {', '.join(memory_tools_found)}")
        else:

...
from radbot.filesystem.tools import (
    read_file, write_file, edit_file as edit_file_func,
    copy as copy_file, list_directory, get_info, search
)
...
def generate_documentation(
    ...
    result = execute_shell_command(
        command=f"pydoc -w {file_path}" if format_arg == "html" else f"pydoc {file_path}",
        timeout=30,  # 30 second timeout
        cwd=os.getcwd()
    )

...
from radbot.agent.execution_agent.models import (
    TaskInstruction, TaskResult, TaskStatus, TaskType
)
...
class ExecutionAgent:
    ...
    def __init__(
        ...
        self.description = description or "Execution agent specialized in implementing specifications"
        self.tools = tools or []
    ...
    def can_transfer_to(self, agent_name):
        ...
        if hasattr(self, "parent_agent") and self.parent_agent and self.parent_agent.name == agent_name:
            return True
...
class AxelExecutionAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx):
        ...
        async for event in parallel_executor.run_async(ctx):
            ...
            if hasattr(event, "actions") and hasattr(event.actions, "state_delta") and event.actions.state_delta and "task_completed" in event.actions.state_delta:
                completed += 1
                yield Event(
                    author=self.name,
                    content=types.Content(

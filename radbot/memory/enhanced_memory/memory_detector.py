...
class MemoryDetector:
    ...
    def __init__(
        ...
        logger.info(f"Memory detector initialized with {len(self.memory_triggers)} memory triggers "
    ...
    def analyze_message(self, message: str) -> Dict[str, Any]:
        ...
        if any(ref in message.lower() for ref in ['last message', 'previous message', 'earlier', 'above']):
    ...
    def extract_information_text(
        ...
        if analysis['trigger_word']:
            ...
            if analysis['trigger_word'] in message:
                ...
                information = message[trigger_index + len(analysis['trigger_word']):].strip()
            ...
            for tag in analysis.get('custom_tags', []):
                information = information.replace(f'#{tag}', '').replace(f'@{tag}', '').strip()
